...
export default function trainer() {
  ...
  let trainer: any = new N.NEATTrainer({
    ...
    sub_population_size : 15, // each sub population has 20 members, so 100 genomes in total
    init_weight_magnitude : 0.55, // randomise initial weights to be gaussians with zero mean, and this stdev.
    mutation_rate : 0.9, // probability of mutation for weights (for this example i made it large)
    mutation_size : 0.05, // if weights are mutated, how much we mutate them by in stdev? (I made it very small for this example)
